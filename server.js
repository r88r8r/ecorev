...
console.log('Clé API présente :', process.env.ANTHROPIC_API_KEY ? 'OUI (' + process.env.ANTHROPIC_API_KEY.substring(0, 14) + '...)' : 'NON — variable manquante !');

function getClient() {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) throw new Error('ANTHROPIC_API_KEY manquante dans les variables Railway');
...
{"note":${maxPts},"mention":"Excellent","positifs":"...","manquants":"...","conseil":"..."}`
    ...
    console.error('Erreur /api/correct :', e.message);
...
app.post('/api/correct-eval', async (req, res) => {
  ...
  try {
    ...
    console.error('Erreur /api/correct-eval :', e.message);
